...
slovar_regular_intervals = {'год': 'year', 'месяц': 'month','число': 'month', 'неделю': 'week', 'день': 'day', 'дня':'days', 'час': 'hour', 'часа':'hours'}
#словарь для дней недели регулярных
slovar_regular_weekdays = {'понедельник': 'Monday', 'вторник': 'Thuesday','сред': 'Wednesday','четверг': 'Thursday','пятниц': 'Friday', 'суббот': 'Saturday','воскресен': 'Sunday','выходн':'Saturday or Sunday'} 
...
def search_regul(message):
   ...
   Out = ' '
   for i in range(len(message)):
       if (re.search('кажд', message[i]) != None or re.search('по', message[i]) != None):
...
def search_fix(message):
    ...
    if T[0]:
            ...
            for i in range(len(line)): 
             ...
             #поищем данный формат dd.dd.dddd
            for i in range(len(line)): 
              D = re.match('\d\d.\d\d.\d\d\d\d', line[i+1])
              if D != None:
                     D = D.string.split(".")
                     Day_Month = int(D[0])
                     Month = int(D[1])
...
#t = "тренировка каждый день в 20:19"
